...
export class ResetPasswordComponent implements OnInit, OnDestroy {
  ...
  private updatePasswordInLocalStorage() {
    try {
      ...
      const userDataStr = localStorage.getItem('souqlyUser');
      ...
      if (userDataStr) {
        ...
        localStorage.setItem('souqlyUser', JSON.stringify(userData));
        
        console.log('Password updated successfully in localStorage');
        this.toastr.info('Password updated locally');
      } else {
        // If no user exists, create a demo user (for testing purposes)
        this.createDemoUserWithNewPassword();
      }
    } catch (error) {
      console.error('Error updating password in localStorage:', error);
      this.toastr.error('Failed to save password locally');
    }
  }
