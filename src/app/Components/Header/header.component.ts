...
import { FavoritesService as FavoritesDepartmentsService } from '../../services/favorites-depatments.service';
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('storage', (event) => {
        ...
        if (event.key === 'souqlyFavoriteDepartments') {
          const favDepts = localStorage.getItem('souqlyFavoriteDepartments');
          this.favoriteDepartmentsCount = favDepts ? JSON.parse(favDepts).length : 0;
        }
      });
    }
